...
// **********************************************************  Macro & Misc. Declarations ************************************************************
        ...
        // ------------------------------------------------- Utility Function ---------------------------------------------------------------
        int compare_points(const void *value_a,const void *value_b);
	...
	//which is of type pointData:
        ...
        *host_points_within_epsilon = 0;
        ...
        // ----------------- Allocate memories on the GPU ------------------------------
            ...
            // -------------- Sort the pointData before passing to the GPU:
            qsort(data,N,sizeof(pointData),compare_points);
...
__global__ void pointEpsilonBruteForce(pointData* all_data_points,int *device_points_within_epsilon,double *device_epsilon)
{
    ...
    for(unsigned long int index = tid; index < N; index++)
    {
    ...
    // Decrementing total_points_within_epsilon by 1 because
    // double counted distance to self:
    total_points_within_epsilon--;
...
int compare_points(const void *value_a,const void *value_b)
{
    pointData first = *(( pointData*)value_a);
	pointData second = *(( pointData*)value_b);
